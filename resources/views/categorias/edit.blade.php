<div x-data="{ modalAbierto: false }">
    ...
    <div x-show="modalAbierto" x-transition.opacity.scale class="fixed inset-0 bg-opacity-50 flex justify-center items-center">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-md relative">
            ...
            <h2 class="text-xl font-bold mb-4 text-center">Editar Categoría</h2>

            <form id="formActualizar" action="{{ route('categoria.update', $categoria->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
            
                <label class="block text-gray-700 font-semibold">Nombre:</label>
                <input type="text" name="nombre" id="nombre" value="{{ $categoria->nombre }}" class="border p-2 w-full rounded focus:ring focus:ring-blue-300">
            
                <label class="block text-gray-700 font-semibold mt-2">Descripción:</label>
                <textarea name="descripcion" id="descripcion" class="border p-2 w-full rounded focus:ring focus:ring-blue-300">{{ $categoria->descripcion }}</textarea>
                ...
                @if ($categoria->imagen)
                    ...
                    <div class="mt-2">
                        <img src="{{ asset('storage/' . $categoria->imagen) }}" width="100" class="rounded shadow-md">
                    </div>
                @endif
